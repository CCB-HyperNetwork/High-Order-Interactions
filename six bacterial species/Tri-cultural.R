...
A <- c(1,2,3,4)
sample_m <- as.numeric(c(data1[1,]))
H0_state <- c(E=(as.numeric(c(data1[1,1]))))

#LS_init_par <- c(-0.6093445,-0.5952629,3.8336319,rep(0.00001,12))



LS_init_par <- c(0.0303749 , 9.3307900, 21.5664292,rep(0.00001,3),
...
H0_par = optim(H0_init_par,H_fn, times = times, pheno = sample_m,effect=t(data1),A=A, state = H0_state,order=4,nstep=10, method = "Nelder-Mead")
#par=H0_init_par
out_tree11 <- get_value(H0_par$par[1:length(LS_init_par)], state = H0_state,effect=t(data1),A=A, times = times,order=4,nstep=10)
