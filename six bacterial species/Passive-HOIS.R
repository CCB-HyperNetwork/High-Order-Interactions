...
ode.sovle.ind <- function(effect,para,nt,order,nstep,LL,A){
...
get_miu = function(miu_par,times,order,effect,nstep,A)
{
  
  nt=seq(min(times),max(times),(max(times)-min(times))/nstep)
  LL <- LMall(NX=1,nt=nt,nstep=nstep,order=order)
  miu2 <- ode.sovle.ind(effect,para=miu_par[1:((order-1)*2)],nt,order,nstep,LL,A)
  miu <- as.numeric(rowSums(miu2))
  return (miu);
}

get_value = function(miu_par,times,order,effect,nstep,A)
{
...
H_fn <- function(par,pheno,effect, times,order,nstep,A){
  miu1 =get_miu(par[1:((order-1)*2)], times = times,order=order,effect=effect,nstep=nstep,A=A)
  sigma1 = SAD1.get_mat(par[(1+(order-1)*2):(4+(order-1)*2)],times = times)
  L0 = c()
...
LS_init_par <- c(0.10087336 ,-0.13878588,  0.06050166, -0.001754410, -0.012625725,0.004970505)
A <- c(1,5)
sample_m <- as.numeric(c(data_EP[1,]))

H0_init_par = c(LS_init_par,0.1, 0.1,0.1, 0.1)
H0_par = optim(H0_init_par,H_fn, times = times, pheno = sample_m,effect=t(data_EP),order=4,nstep=13, method = "Nelder-Mead",A=A)

out_EP1 <- get_value(H0_par$par[1:length(LS_init_par)],times = times,order=4,effect=t(data_EP),nstep=13, A=A)
