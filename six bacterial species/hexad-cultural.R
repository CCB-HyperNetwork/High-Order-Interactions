...
ode.sovle.ind <- function(effect,para,nt,order,nstep,LL,connect){
  stp <- (max(times)-min(times))/nstep
  diag(connect) <- 1
  index <- which(connect[1,]==1)
...
get_miu = function(miu_par,state,times,order,pheno,effect,nstep,connect)
{
  ...
  nn <- length(which(connect[1,]==1))
  miu2 <- ode.sovle.ind(effect,para=miu_par[4:(3+(order-1)*(nn+1))],nt,order,nstep,LL,connect)
  miu4 <- cbind(miu1,miu2[,2:length(miu2[1,])])
  miu <- as.numeric(rowSums(miu4))
  return (miu);
}


get_value = function(miu_par,state,times,effect,order,nstep,connect)
{
  parameters <- c(r1 = miu_par[1],
                  a1 = miu_par[2],
                  k1 = miu_par[3]
  ...
  nn <- length(which(connect[1,]==1))
  miu2 <- ode.sovle.ind(effect,para=miu_par[4:(3+(order-1)*(nn+1))],nt,order,nstep,LL,connect)
  miu4 <- as.matrix(miu2[,2:length(miu2[1,])])
  value <- matrix(NA,ncol = (length(miu2[1,])),nrow = length(times))
  value[,1] <-miu1
  value[,2:length(miu2[1,])] <-miu4
  return (value);
}
...
H_fn <- function(par,pheno,effect, state,times,order,nstep,connect){
  nn <- length(which(connect[1,]==1))
  miu1 =get_miu(par[1:((order-1)*(nn+2)+3)], state = state,times = times,order=order,effect=effect,pheno=pheno,nstep=nstep,connect)
  sigma1 = SAD1.get_mat(par[((order-1)*(nn+2)+4):((order-1)*(nn+2)+9)],times = times)
  L0 = c()
  L0 = sum(dmvnorm(pheno,miu1,sigma1,log = T))
  return(-L0)
}
sample_m <- as.numeric(c(data31[1,]))
H0_state <- c(E=(as.numeric(c(data31[1,1]))))

#LS_init_par <- c(-0.6093445,-0.5952629,3.8336319,rep(0.00001,12))
lasso61 <- matrix(0,ncol = 16,nrow = 16)
lasso61[1,2:16] <- 1
rownames(lasso61) <- c("1","2","3","4","5","6","23","24","25","26","34","35","36","45","46","56")
colnames(lasso61) <- c("1","2","3","4","5","6","23","24","25","26","34","35","36","45","46","56")



LS_init_par <- c(0.066823 , 4.245165, 15.360690,rep(0.00001,3),
                 -0.05032153,  0.02682032 , -0.01575637,
                 -0.004700969, 0.017082511, -0.019824097,
                 -0.0051247949 , 0.0009280299, -0.0178621581,
                 -0.086970354, -0.122669375,  0.080482920,
                 0.007278326, -0.050020427,  0.032704430,
                 -0.02509914, -0.02712465,  0.02699475,
                 0.022341209,0.024114919,-0.041621631,
                 0.019435340,-0.072920937,0.022321871,
                 -0.0213525028 , 0.0197976356 , 0.0040073410,
                 -0.02526427 , 0.03662988 ,  -0.00108556,
                 0.183793485, -0.053298565,  0.100984824,
                 0.04144180 ,-0.09357984, -0.01209475,
                 -0.0213525028 , 0.0197976356 , 0.0040073410,
                 -0.02526427 , 0.03662988 ,  -0.00108556,
                 0.019435340,-0.072920937,0.022321871)
...
H0_par = optim(H0_init_par,H_fn, times = times, pheno = sample_m,effect=t(data31),connect=lasso61, state = H0_state,order=4,nstep=10, method = "Nelder-Mead")
#par=H0_init_par
out_tree31 <- get_value(H0_par$par[1:length(LS_init_par)], state = H0_state,effect=t(data31),connect=lasso61, times = times,order=4,nstep=10)
