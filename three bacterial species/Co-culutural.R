...
get_miu = function(miu_par,state,times,order,pheno,effect,nstep,A)
{
  ...
  miu2 <- ode.sovle.ind(effect,para=miu_par[4:(3+(order-1)*2)],nt,order,nstep,LL,A)
  miu3 <- cbind(miu1,miu2[,2])
...
get_value = function(miu_par,state,times,effect,order,nstep,A)
{
  ...
  LL <- LMall(NX=1,nt=nt,nstep=nstep,order=order)
  miu2 <- ode.sovle.ind(effect,para=miu_par[4:(3+(order-1)*2)],nt,order,nstep,LL,A)
  miu3 <- as.numeric(miu2[,2])
  value <- matrix(NA,ncol = 2,nrow = length(times))
  value[,1] <-miu1
  value[,2] <-miu3
  return (value);
}
...
H_fn <- function(par,pheno,effect, state,times,order,nstep,A){
  miu1 =get_miu(par[1:(3+(order-1)*2)], state = state,times = times,order=order,effect=effect,pheno=pheno,nstep=nstep,A=A)
  sigma1 = SAD1.get_mat(par[(4+(order-1)*2):(7+(order-1)*2)],times = times)
  L0 = c()
...
A <- c(4,5)
sample_m <- as.numeric(c(data[4,]))
H0_state <- c(E=( as.numeric(c(data[4,1]))))

#LS_init_par <- c(0.066823 , 4.245165, 15.360690,rep(0.00001,3),0.07197402,-0.14800011,-0.14002316)
LS_init_par <- c(0.066823 , 4.245165, 15.360690,rep(0.00001,6))
H0_init_par = c(LS_init_par,0.1, 0.1,0.1, 0.1)
H0_par = optim(H0_init_par,H_fn, times = times, pheno = sample_m,effect=t(data), state = H0_state,order=4,nstep=13, method = "Nelder-Mead",A=A)

out1 <- get_value(H0_par$par[1:length(LS_init_par)], state = H0_state,effect=t(data), times = times,A=A,order=4,nstep=13)
